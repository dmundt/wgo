...
func (h *Harness) CreateGraph() (*graph.Graph, error) {
	dot := graph.NewGraph()
	...
	for _, connector := range h.Connectors {
		...
		extra, err := getAdditionalComponentTable(h, connector)
		if err != nil {
			return nil, err
		}
		...
		if len(connector.Loops) > 0 {
			...
			if connector.PortsLeft {
				...
				return nil, fmt.Errorf("No side for loops")
			}
	...
	for _, cable := range h.Cables {
		...
		extra, err := getAdditionalComponentTable(h, cable)
		if err != nil {
			return nil, err
		}
		rows = append(rows, extra...)
	...
	if err := h.applyTweak(dot); err != nil {
		return nil, err
	}
